...
bookmark_border
Genkit includes a plugin that helps you deploy your flows to Cloud Functions for Firebase. Flows, once deployed, are available as HTTPS endpoints and accessible as callable functions using the Cloud Functions client libraries.
...
The rest of this page assumes that you've selected to write your functions in TypeScript, but you can also deploy your Genkit flows if you're using JavaScript.
...
For your flows to be deployable, you will need to make some small changes to how you define them. The core logic will remain the same, but you will add some additional information to make them smoothly deployable and more secure once deployed.
...
The following sections describe the changes you need to make before you can deploy it.

Define flows with onFlow
Instead of defining your flow with Genkit.defineFlow(), use the Firebase plugin's onFlow() function. Using this function wraps your flow logic in a Cloud Functions request handler, similar to onCall.
...
Set a CORS policy
If you'll access your flow from a web app (which you will do in the Try the deployed flow section), in the httpsOptions parameter, set a CORS policy:
...
In the Authentication section of the Firebase console, enable the Google provider, which you will use in this example.
